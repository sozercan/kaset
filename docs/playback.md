...
|--------|------------------|---------|
| Media control bootstrap | document start | Wraps `navigator.mediaSession.setActionHandler` so Kaset owns play/pause across route changes, seek-forward/back stay native-owned, and next/previous can be bridged into Swift when needed. |
